...
// Preencha SOMENTE com a tabela oficial da Tata Storys quando ela for fornecida.
const SIZE_GUIDE = {
...
function replaceOriginalImages() {
  ...
  if (editorial) {
    ...
    const img = new Image();
    img.src = ASSETS.editorial;
    img.alt = "Thamires Santos em fotografia de apresentação";
    img.width = 722;
    img.height = 1600;
    img.className = "tata-original editorial-original";
    img.decoding = "async";
    img.fetchPriority = "high";
    const label = document.createElement("span");
    label.className = "editorial-original__label";
    label.textContent = "THAMIRES SANTOS / TATA STORYS";
    editorial.append(img, label);
  }
  ...
  slots.slice(0, 3).forEach((slot, i) => {
    const [src, alt, labelText] = items[i];
...
function buildFitLab() {
  ...
  <div class="fit-wrap">
    <header class="fit-head">
      ...
      <h2 id="fitTitle">Meça primeiro.<br><span>Escolha com mais contexto.</span></h2>
      <p>O manequim mostra onde medir cintura e quadril. P, M, G e GG já estão preparados; a indicação automática só será liberada quando a tabela oficial em centímetros for cadastrada.</p>
    </header>

    <div class="fit-grid">
      <article class="fit-card">
        <div class="fit-copy"><small>PRODUTO REAL / IMAGEM ORIGINAL</small><h3>Cinta em destaque</h3><p>A imagem é usada sem alterar a pessoa. Confirme modelo, tamanho e disponibilidade no atendimento.</p></div>
        <figure class="corset-real"><img src="${ASSETS.cinta}" alt="Cinta modeladora em imagem original enviada para Tata Storys" width="722" height="1600" loading="lazy" decoding="async"><figcaption>Imagem original • sem recompressão pelo código</figcaption></figure>
      </article>

      <article class="fit-card">
        <div class="fit-copy"><small>MODELO 360° / ILUSTRATIVO</small><h3>Gire e veja onde medir</h3><p>Arraste para os lados. O manequim é apenas um guia visual, não representa o corpo da cliente.</p></div>
      ...
      <article class="fit-card fit-card--guide">
        <div class="fit-copy"><small>P / M / G / GG</small><h3>Guia de tamanho</h3><p>Selecione um tamanho ou informe suas medidas. Sem a tabela oficial, o site não arrisca sugerir um tamanho errado.</p></div>
        <div class="size-layout">
          <div>
            <div class="sizes" id="sizes">${["P", "M", "G", "GG"].map(s => `<button type="button" data-size="${s}">${s}</button>`).join("")}</div>
            <div class="size-status" id="sizeStatus"><strong>Tabela oficial aguardando cadastro.</strong><br>Nenhuma faixa em centímetros foi inventada.</div>
          ...
          <form id="measureForm" class="measure-form">
            <div class="measure-fields"><label>Cintura (cm)<input id="waistCm" name="waist" type="number" min="30" max="200" step=".5" inputmode="decimal" placeholder="Ex.: 78"></label><label>Quadril (cm)<input id="hipCm" name="hip" type="number" min="40" max="220" step=".5" inputmode="decimal" placeholder="Ex.: 104"></label></div>
            ...
            <div class="measure-result" id="measureResult" aria-live="polite">Digite suas medidas. O sistema só indica P/M/G/GG quando houver faixas oficiais cadastradas.</div>
            <small class="fit-note">Guia comercial e visual. Não faz avaliação médica, corporal ou fisiológica.</small>
          </form>
        </div>
      </article>
    </div>
...
function initRotation() {
  const stage = $f("#modelStage");
  const model = $f("#model3d");
